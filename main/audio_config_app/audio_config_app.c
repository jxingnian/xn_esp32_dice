 ...
 * @LastEditTime: 2025-12-02 12:37:27
                            ...
                            void *user_ctx)
{
    ...
    *cfg = AUDIO_MANAGER_DEFAULT_CONFIG();
    ...
    // ========== AFE（音频前端处理）配置 ==========
    cfg->afe_config.aec_enabled = false;       // 启用回声消除（AEC）
    cfg->afe_config.ns_enabled = false;        // 启用降噪（NS）
    cfg->afe_config.agc_enabled = false;       // 启用自动增益控制（AGC）
